...
$(()=>{
    ...
    hotFunction().p();
    $('.hot .hot_more').click(()=>{hotFunction().p()});
    $('.hot .gallRankTab').click(()=>{hotFunction().g('g')});
    $('.hot .mgallRankTab').click(()=>{hotFunction().g('m')});
    $('.hot .minigallRankTab').click(()=>{hotFunction().g('mi')});
});
...
const hotFunction = () => {
    ...
    const pageLoadAjax = (jsonData) => {
        return new Promise(function(resolve, reject){
            $.ajax({
                url:'/GCInside/mainIndex/hotGetList',
                method: 'POST',
                contentType: 'application/json',
                data: JSON.stringify(jsonData),
                dataType:'json',
    ...
    const pageLoad = async (gall_type, page) => {
        // 페이지 리스트가 있을 경우 중지
        if($('.hot .'+gall_type+'_'+page+' li').length > 0) return;
    ...
    const btnChange = (gall_type, page) => {
        ...
        // gall_type 에 따른 버튼 on 변경
        $('.hot .rank_'+ (gall_type == 'g' ? 'gall' : gall_type == 'm' ? 'mgall' : 'mini')).addClass('on');

        // page에 따른 값 변경 및 버튼 이름 변경
        $('.hot .hot_more').data('gall_type', gall_type).data('page', page).text(
            page == 1 ? '11위 - 20위' : page == 2 ? '21위 - 30위' : page == 3 ? '31위 - 40위' : page == 4 ? '41위 - 50위' : '1위 - 10위'
        );
    };
    ...
    const pageOn = (gall_type, page) => {
        ...
        // 해당 페이지 열기
        $('.hot .'+gall_type+'_'+page).toggle();
    };

    // 페이지 이동 및 로드 함수
    // mode -> 이동 하는 페이지 숫자 : {0: 현제 페이지 로드, 1: 페이지 앞으로 이동}
    const pageMove = () => {
        const gall_type = $('.hot .hot_more').data("gall_type"); // 갤러리 타입
        const page = $('.hot .hot_more').data("page"); // 현재 페이지
    ...
    const gallMove = (gall_type) => {
        ...
        $('.hot .hot_more').data("gall_type", gall_type);
        $('.hot .hot_more').data("page", 0);
