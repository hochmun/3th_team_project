...
$(function() {
    ...
    $("input[type=file]").on("change", function() {
        var r = $(this).data("purpose")
          , e = $("input[name=token]")[0].value
          , t = $(this)[0].files[0];
        if (!t)
            return alert(i18n("EDIT_USERINFO_SELECT_FILE"));
        var n = new FormData;
        if (n.append("upload", t),
        n.append("token", e),
        n.append("purpose", r),
        "defaultImage" === r) {
            if ($("#gallery_image_preview > img").length >= imagelimit)
                return;
            var i = $("#gallery_image_preview")
        }
        $.ajax({
            url: "/b/upload",
            type: "POST",
            ...
            i.append(a),
            o()) : "galleryIcon" === r && ($("#gallery_icon_preview").empty(),
            (a = $("<img />")).attr("src", t),
            $("#gallery_icon_preview").append(a))) : alert(e.message)
        }).fail(function(e) {
            try {
